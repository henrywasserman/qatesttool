...
import com.pqi.responsecompare.sql.MySqlDbManager;
...
public class EdiToMySql extends Request {

    static final Logger logger = Logger.getLogger(EdiToMySql.class);
    ...
    public EdiToMySql(TestCase test) throws Exception {
        super(test);
    }

    public EdiToMySql() throws Exception {
    ...
    public void sendRequest() throws Exception {
        ...
        String table = PropertiesSingleton.Instance.getProperty("edi_table.MYSQL");

        props.setProperty("Host.MYSQL", "192.168.1.70");
        props.setProperty("database.MYSQL", "sqadatabase");
        MySqlDbManager dbManager = new MySqlDbManager(props);
        ResultSet resultSet = null;
        String query = "SELECT * FROM " + table + " ORDER BY x_file_create_date DESC";
        logger.info(query);
        ...
        try {
            ...
            dbManager = new MySqlDbManager(props);
                    ...
                    if (object instanceof DTPType) {
                        ...
                        if (Maps.Instance.dateTimeQualifier.containsKey(key)) {
                            if (key.equals("357")) {
                                ...
                                DateFormat newFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.0");
                                DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd");
                                Date date = formatter.parse(dtp.getDateTimePeriodFormat());
...
//                                            dtp.getDateTimePeriodFormat());
                ...
                sql.append("TESTCASE " + Maps.Instance.ediResults.get("x_last_name") +
                        ...
                        "  RUN_MYSQLSERVER_EXECUTE_SQL 192.168.1.70\\sqadatabase, edi_test_data\n" +
                        "  SELECT " + sb.toString()  +
                        "  FROM sqadatabase." +  table +
                        "  WHERE x_last_name = '" +
                        Maps.Instance.ediResults.get("x_last_name") + "' " +
                        "  AND x_ssn = '" +
                        Maps.Instance.ediResults.get("x_ssn") + "' " +
                        ...
                        "  ORDER BY x_file_create_date DESC LIMIT 1\n" +
                        "  COMPARE\n");
                ...
                FileUtils.writeStringToFile(new File("data" + sep
                        + "consult" + sep
                        + "mp" + sep
                        + "edi_testcases.req"),
                        sql.toString(), true);
                ...
                json.append("{");
                insertSql.append("INSERT INTO sqadatabase.edi_test_data " +
                        "(column_1,\n" +
                        "x_filename,\n" +
                        "x_confidentiality_code,\n" +
                        "x_dob,\n" +
                        "x_enrolled_by,\n" +
                        "x_entity_identification_code,\n" +
                        "x_entity_type_qualifier,\n" +
                        "x_first_name,\n" +
                        "x_gender_code,\n" +
                        "x_handicap_indicator,\n" +
                        "x_hic_number,\n" +
                        "x_last_name,\n" +
                        "x_maintenance_type_code,\n" +
                        "x_mass_health_id,\n" +
                        "x_middle_name,\n" +
                        "x_plan_end_date,\n" +
                        "x_plan_start_date,\n" +
                        "x_primary_language_spoken,\n" +
                        "x_primary_language_written,\n" +
                        "x_race_or_ethnicity_code,\n" +
                        "x_rating_category,\n" +
                        "x_ssn) VALUES(");
                ...
                for (String column:columns) {
                    ...
                    if (column.equals("x_dob")) {
                        insertSql.append("STR_TO_DATE('" + Maps.Instance.ediResults.get(column) + "','%Y-%m-%d %H:%i:%s.0'),\n");
                    }
                    else if (column.equals("x_plan_end_date") ||
                        column.equals("x_plan_start_date")) {
                        insertSql.append("STR_TO_DATE('" + Maps.Instance.ediResults.get(column) + "','%Y-%m-%d %H:%i:%s.0'),\n");
                    }
                    else if(column.equals("column_1")) {
                        insertSql.append("null,");
                    }
                    else if (column.equals("x_file_create_date")) {
                        insertSql.append("CURRENT_DATE,");
                    }
                    else {
                        insertSql.append("\"" + Maps.Instance.ediResults.get(column) + "\",\n");
                    }
                }
