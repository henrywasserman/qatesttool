...
import com.pqi.responsecompare.sql.TeradataDbManager;
...
public class RunTeradataExecute extends Request {
	Properties props = Utilities.Instance.getTestProperties();

	String creativeIds = new String();

	public RunTeradataExecute(TestCase test) throws Exception {
		super(test);
	}

	@Override
	public void sendRequest() throws Exception {
		TeradataDbManager dbManager = null;
		...
		try {
			if (!url.isEmpty()) {
				...
				props.setProperty("teradata-host", properties[0].trim());
				props.setProperty("teradata-dbname", properties[1].trim());
			...
			dbManager = new TeradataDbManager(props);
