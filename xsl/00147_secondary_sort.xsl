...
<xsl:output method="xml"/>
	...
	<xsl:template match="@*|last_update">
	    <xsl:copy>
		    <xsl:apply-templates select="@*"/>	
	    </xsl:copy>
    </xsl:template>    
	...
	<xsl:template match="root">
  		<xsl:copy>
    		<xsl:apply-templates select="node()">
      			<!-- sort first on the displayText -->
      			<xsl:sort select="displayText" data-type="text" order="ascending" />
      			<!-- for lines with the same displayText, sort by id -->
