...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="@*|folder_name">
		<xsl:copy>
			<xsl:apply-templates select="folder_name" />
		</xsl:copy>
	</xsl:template>

	<xsl:template match="@*|folder_message_count">
		<xsl:copy>
			<xsl:apply-templates select="folder_message_count" />
		</xsl:copy>
	</xsl:template>

	<xsl:template match="@*|folder_unread_message_count">
		<xsl:copy>
			<xsl:apply-templates select="folder_unread_message_count" />
		</xsl:copy>
	</xsl:template>
