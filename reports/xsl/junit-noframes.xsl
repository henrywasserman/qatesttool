...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="testDir">
        <xsl:value-of select="concat(system-property('test.dir'),'/')"/>
    </xsl:variable>

    <xsl:variable name="testExt">
        <xsl:value-of select="system-property('test.ext')"/>
    </xsl:variable>

    <xsl:variable name="localDir">
        <xsl:value-of select="system-property('local.dir')"/>
    </xsl:variable>

    <xsl:variable name="finalDir">
        <xsl:choose>
            <xsl:when test="$localDir != ''">
                <xsl:value-of select="concat($localDir,'/',$testDir)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$testDir"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
            ...
            <body>
                <a target="_blank">
                    <img width="200" height="80"
                         style="-webkit-user-select: none;background-position: 0px 0px, 10px 10px;background-size: 20px 20px;,linear-gradient(45deg, #eee 25%, white 25%, white 75%, #eee 75%, #eee 100%);"
                         src="/userContent/images/SQA_Logo_Web.png"/>
                </a>
                <p></p>
                <i></i>
                <br></br>
                ...
                <td>
                    <xsl:choose>
                        <xsl:when test="contains(@name,'Open Bug') and contains(@name,'Desc')">
                            ...
                            <font color="green">Open Bug</font>
                            <br></br>
                            <a onclick="window.open('{translate(substring-before(substring-after(@name,'Open Bug'),'Desc'),'&#32;','')}','_blank','location=yes,scrollbars=yes,status=yes');">
                                <font color="blue">
                                    <u>
                        ...
                        <xsl:when test="contains(@name,'Closed Bug') and contains(@name,'Desc')">
                            <xsl:value-of select="substring-before(@name,'Closed Bug')"/>
                            <font color="orange">Closed Bug</font>
                            <br></br>
                            <a onclick="window.open('{translate(substring-before(substring-after(@name,'Closed Bug'),'Desc'),'&#32;','')}','_blank','location=yes,scrollbars=yes,status=yes');">
                                <font color="blue">
                                    <u>
                                        <xsl:value-of select="substring-after(@name,'Desc')"/>
                                    </u>
                        ...
                        <xsl:when test="contains(@name,'Open Bug')">
                            <xsl:value-of select="substring-before(@name,'Open Bug')"/>
                            <br></br>
                            <a href="{translate(substring-after(@name,'Open Bug'),'&#32;','')}">
                            </a>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="@name"/>
    ...
    <xsl:template name="display-failures">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="disableCompare"
                              select="contains(@message,'disableCompare***')"/>
                ...
                <table>
                    <tr>
                        <td>
                            <a target="_request"
                               href="/job/{$JobName}/{$BuildNumber}/artifact/data/consult/{$finalDir}request{$dataFile}.html">
                                Request
                            </a>
                        </td>
                        <td>
                            <a target="_response"
                               href="/job/{$JobName}/{$BuildNumber}/artifact/data/consult/{$finalDir}response{$dataFile}.{$testExt}">
                                Response
                            </a>
                        </td>
                        <td>
                            <a target="_expectedresponse"
                               href="/job/{$JobName}/{$BuildNumber}/artifact/data/consult/{$finalDir}expectedresponse{$dataFile}.{$testExt}">
                                Expected Response
                            </a>
                        </td>
                        <xsl:call-template name="setupCompare">
                            <xsl:with-param name="dataFile" select="$dataFile"/>
                        </xsl:call-template>

                        <td>
                            <a target="_differences"
                               href="/job/{$JobName}/{$BuildNumber}/artifact/data/consult/{$finalDir}response{$dataFile}_diff.html">
                                Differences
                            </a>
                        </td>
                        <!-->
    ...
    <xsl:template name="setupCompare">
        <xsl:param name="dataFile"/>
        <xsl:choose>
            <xsl:when test="contains(@message,'disableCompare***')">
                <td>
                    <a href="javascript:window.open('toobig.html', 'File Diff', 'width=800,height=400');">
                        Compare
                    </a>
                </td>
            ...
            <xsl:otherwise>
                <td>
                    <a href="javascript:lhs_url='/job/{$JobName}/{$BuildNumber}/artifact/data/consult/{$finalDir}response/{$dataFile}.json';rhs_url='/job/{$JobName}/{$BuildNumber}/artifact/data/consult/{$finalDir}expectedresponse/{$dataFile}.json';
